...
export default defineEventHandler(async (event) => {
  ...
  const [result] = await pool.execute<any>('UPDATE `user` SET Password = ?, ResetPasswordToken = NULL, ResetPasswordExpiry = NULL WHERE ResetPasswordToken = ? AND ResetPasswordExpiry > UTC_TIMESTAMP()', [await hashPassword(password), hashToken(token)])
