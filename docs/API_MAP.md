...
| GET | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `employee`, `employee_deployment`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Returns enrolled employee totals plus one daily attendance row per employee/date, the DTR-specific Regular/Reliever type, active agency shift codes, and persisted `IsWDO` / `WDODays` payroll markers. Days worked count only payable/time-stamped attendance; each saved row contributes its snapshotted shift-code `WorkdayCount`, so a configured 2-day straight duty contributes 2 Total Days. Saved Absent, Rest Day, Leave, and Reliever placeholders remain retrievable but do not count. This drives the clickable cutoff matrix. |
