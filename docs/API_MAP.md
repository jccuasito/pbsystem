...
| GET | `/api/employees/deployments` | `employee_deployment`, `employee`, `client_rate`, `payroll_rate`, `agency_position`, `agency`, `position`, `site`, `site_shift`, `shift_code`, `client` | `app/pages/employees/deployment-history/index.vue` | Session required. Returns `{ items, agencies, employees, clientRates, sites, shiftCodes }`; the page groups one visible row per employee and filters that current assignment by agency, client, or site, with a Details timeline for the person's deployment status/history. Employee selectors use integer `EmployeeID`, displayed as formatted `EMP-0001`, while `EmployeeNumber` remains optional. Shift lookups expose `SiteShiftID` and `SiteID`. |
...
| DELETE | `/api/attendance/dtr/:id` | `attendance_dtr`, `attendance` | `app/pages/attendance/daily-time-records/index.vue` | Session required. Deletes only an empty DTR batch; batches that already contain attendance are protected. |
...
| GET | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `employee`, `employee_deployment`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Returns enrolled employee totals plus one daily attendance row per employee/date, the DTR-specific Regular/Reliever type, and the agency's active shift codes. This drives the clickable cutoff matrix. |
...
| PUT | `/api/attendance/dtr/:id/employees` | `attendance_dtr`, `attendance_dtr_employee` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body `{ EmployeeID, DeploymentType }`; changes only this DTR employee's Regular/Reliever type. |
| POST | `/api/attendance/dtr/:id/apply-shift` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body `{ EmployeeID, ShiftCodeID, OnlyEmpty }`; applies an active agency shift to every day in the cutoff, including its configured regular hours and regular-OT cap. It adopts legacy unbatched attendance into this DTR; `OnlyEmpty` preserves daily exceptions already in this DTR. |
| POST | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Upserts one employee/date cell; body includes `EmployeeID`, optional agency-owned `ShiftCodeID`, date/time, attendance status, remarks, and all attendance hour fields. `TimeIn`/`TimeOut` accept copy-pasteable `YYYY-MM-DD HH:mm` (or ISO `T`) format. When an enabled-ND shift is selected, the server calculates `NightDiffHours` from the actual time overlap with that shift code's ND window. The UI clears shift, timestamps, and hours for Absent, Rest Day, and On-Leave. |
