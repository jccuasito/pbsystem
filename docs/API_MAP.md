...
| POST | `/api/attendance/dtr/:id/apply-shift` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body `{ EmployeeID, ShiftCodeID, OnlyEmpty }`; atomically applies an active agency shift to every calendar day in the cutoff, including its configured regular hours and regular-OT cap. It normalizes MySQL DATE/DATETIME values before generating the cutoff dates, adopts legacy unbatched attendance into this DTR, and `OnlyEmpty` preserves daily exceptions already in this DTR. |
