...
| GET | `/api/employees/deployments` | `employee_deployment`, `employee`, `client_rate`, `payroll_rate`, `agency_position`, `agency`, `position`, `site`, `site_shift`, `shift_code`, `client` | `app/pages/employees/deployment-history/index.vue` | Session required. Returns `{ items, agencies, employees, clientRates, sites, shiftCodes }`; `agencies` contains every active organization agency for the history filter, including agencies with no deployments. Employee selectors use integer `EmployeeID`, displayed as formatted `EMP-0001`, while `EmployeeNumber` remains optional. Shift lookups expose `SiteShiftID` and `SiteID`. |
...
| GET | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `employee`, `employee_deployment` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Returns the DTR attendance workspace rows: enrolled employee, Regular/Reliever type, days, and totals for every attendance-hours column. |
| GET | `/api/attendance/dtr/:id/employees` | `attendance_dtr`, `employee`, `agency_position`, `position` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Query `search`; lists active employees only from the DTR agency for the employee picker. |
| POST | `/api/attendance/dtr/:id/employees` | `attendance_dtr`, `employee`, `client_rate`, `payroll_rate`, `employee_deployment`, `attendance_dtr_employee` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body `{ EmployeeID, DeploymentType: 'Regular' | 'Reliever' }`; creates/reuses a cutoff site deployment and enrolls the employee in the DTR without changing their employee master agency. |
| PUT | `/api/attendance/dtr/:id/employees` | `attendance_dtr`, `attendance_dtr_employee`, `employee_deployment` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body `{ EmployeeID, DeploymentType }`; changes the DTR-linked deployment to Regular or Reliever so Deployment History reflects the actual attendance classification. |
| POST | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Body includes `EmployeeID`, optional agency-owned `ShiftCodeID`, date/time, attendance status, remarks, and all attendance hour fields. Requires an employee already enrolled in the DTR and a date inside the cutoff. The selected shift is retained on the attendance row. |
