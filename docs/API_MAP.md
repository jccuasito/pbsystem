...
| GET | `/api/attendance/dtr/:id/records` | `attendance_dtr`, `attendance_dtr_employee`, `attendance`, `employee`, `employee_deployment`, `shift_code` | `app/components/DtrAttendanceWorkspace.vue` | Session required. Returns enrolled employee totals plus one daily attendance row per employee/date, the DTR-specific Regular/Reliever type, active agency shift codes, and persisted `IsWDO` / `WDODays` payroll markers. This drives the clickable cutoff matrix. |
