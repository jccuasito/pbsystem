...
test('deployment shows agency shifts, follows the exact rate site, resets dependent selections, and refreshes on opening', async () => {
  let loads = 0
  ...
  const state = scope.run(() => evaluate(source + '\nmodule.exports={openNewDeployment,form,employees,availableSiteRates,availableShifts,employeeOptions,onEmployeeChanged,onSiteRateChanged,onSiteChanged,canSave};', {
    ...
    $fetch: async () => { loads++; return {
      ...
      siteRates: [{ SiteRateID: 1, AgencyID: 3, AgencyPositionID: 3, ClientID: 1, SiteID: 1 }, { SiteRateID: 2, AgencyID: 3, AgencyPositionID: 4, ClientID: 1, SiteID: 1 }, { SiteRateID: 3, AgencyID: 1, AgencyPositionID: 1, ClientID: 1, SiteID: 1 }], sites: [{ SiteID: 1, ClientID: 1 }],
  ...
  try {
    ...
    assert.equal(state.availableSiteRates.value.length, 1)
    assert.equal(state.availableSiteRates.value[0].SiteRateID, 1)
    state.form.value.SiteRateID = '2'; assert.ok(!state.canSave.value)
    state.form.value.SiteRateID = '1'
    // A refreshed employee position immediately invalidates the old rate.
    state.employees.value[0].AgencyPositionID = 4
    assert.equal(state.availableSiteRates.value[0].SiteRateID, 2)
    assert.ok(!state.canSave.value)
    state.employees.value[0].AgencyPositionID = 99
    assert.equal(state.availableSiteRates.value.length, 0)
    ...
    state.onSiteRateChanged(); assert.equal(state.form.value.SiteID, '1')
    state.onEmployeeChanged(); assert.equal(state.form.value.SiteRateID, '')
...
test('MySQL deployment links agency shifts atomically, reuses mappings, and rejects invalid selections', { skip: process.env.DEPLOYMENT_TEST_DATABASE !== '1' }, async () => {
  ...
  try {
    ...
    const tables = {
      ...
      employee_deployment: 'DeploymentID INT AUTO_INCREMENT PRIMARY KEY, EmployeeID INT, SiteRateID INT, SiteID INT, SiteShiftID INT, DeploymentType VARCHAR(20), StartDate DATE, EndDate DATE, Remarks TEXT, CreatedBy INT, IsPermanentSite TINYINT DEFAULT 1',
    }
    ...
    await c.query("INSERT INTO site_rate VALUES (3,10,3,'Active'),(1,10,1,'Active'),(4,10,4,'Active')")
    ...
    const body = { EmployeeID: 7, SiteRateID: 3, SiteID: 10, ShiftCodeID: 7, DeploymentType: 'Regular', StartDate: '2026-09-19' }
    // Same agency but wrong position must fail before any deployment or link write.
    await assert.rejects(api.createDeployment({ body: { ...body, SiteRateID: 4 } }), error => error.statusCode === 400 && /position saved in Employee List/.test(error.message))
    ...
    for (const invalid of [{ ShiftCodeID: 1 }, { ShiftCodeID: 8 }, { SiteID: 11 }, { SiteRateID: 1 }, { ShiftCodeID: '' }, { EndDate: '2026-09-18' }]) {
    ...
    // Existing callers can still submit a saved site-linked shift; keep the agency validation.
