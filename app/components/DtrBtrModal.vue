...
const sheetCellTitle=(employee:BtrEmployee,date:string)=>relieverDayTitle(employee,date)+' · REST: '+hours((gridCell(employee.EmployeeID,date)?.units||0)/100)+' h'+(gridCell(employee.EmployeeID,date)?.rows.map(row=>' · Covered by '+row.name+': '+hours(row.hours)+' h').join('')||'')
...
<template>
  <Teleport to="body">
    <dialog ref="dialog" class="btr-dialog" aria-labelledby="btr-title" @cancel.prevent="close" @click="($event.target===dialog)&&close()">
      ...
      <div class="btr-body">
        <div class="btr-toolbar"><p>Select the day of the employee taking a break, choose their <strong>reliever</strong>, then Save BTR.</p><div>
        ...
        <template v-if="data">
          ...
          <p class="btr-grid-hint">BTR = covered someone else's break. REST = had their own break covered. One row per employee.</p>
          <div class="btr-grid-legend"><span><i class="btr-legend-filled"></i>BTR · covered a break</span><span><i class="btr-legend-rest"></i>REST · took a break</span><span><i class="btr-legend-pending"></i>Unsaved change</span><span><i class="btr-legend-warning"></i>Check attendance</span></div>
          <div class="btr-grid-wrap" tabindex="0" aria-label="BTR and REST cutoff grid, scroll to see all days">
            <table class="btr-grid"><colgroup><col class="btr-person-col"><col v-for="date in cutoffDays" :key="date" class="btr-day-col"><col class="btr-total-col"><col class="btr-total-col"></colgroup>
              <thead><tr><th scope="col" rowspan="2" class="btr-grid-person">Name of personnel / E-ID</th><th v-for="date in cutoffDays" :key="date" scope="col" :title="dateLabel(date)">{{Number(date.slice(8))}}</th><th scope="col" rowspan="2" class="btr-grid-total btr-total-covered">BTR<br>hours</th><th scope="col" rowspan="2" class="btr-grid-total">REST<br>hours</th></tr><tr><th v-for="date in cutoffDays" :key="date" scope="col" class="btr-hours-head">HRS</th></tr></thead>
              <tbody><tr v-for="employee in sheetPeople" :key="employee.EmployeeID">
                ...
                <td v-for="date in cutoffDays" :key="date" class="btr-grid-day">
                  <button type="button" :disabled="loading||saving||reading" :class="{'is-pending':relieverDay(employee.EmployeeID,date)?.pending||gridCell(employee.EmployeeID,date)?.pending,'has-warning':relieverDayWarning(employee.EmployeeID,date)||(!!gridCell(employee.EmployeeID,date)&&!!attendanceWarning(employee.EmployeeID,date))}" :aria-label="sheetCellTitle(employee,date)" :title="sheetCellTitle(employee,date)" @click="openCell(employee,date)">
                    ...
                    <span v-if="gridCell(employee.EmployeeID,date)" class="btr-day-portion btr-day-rest">
                      <strong>{{hours(gridCell(employee.EmployeeID,date)!.units/100)}}</strong>
                      <small class="btr-rest-label">REST</small>
                    </span>
                    ...
                    <small v-if="relieverDay(employee.EmployeeID,date)?.pending||gridCell(employee.EmployeeID,date)?.pending" class="btr-pending-dot" aria-label="Unsaved change">●</small>
                  </button>
                </td>
                <td class="btr-grid-total btr-total-covered">{{hours(employeeBtrTotal(employee.EmployeeID))}}</td><td class="btr-grid-total">{{hours(personTotal(employee.EmployeeID))}}</td>
              </tr><tr v-if="!sheetPeople.length"><td :colspan="cutoffDays.length+3" class="btr-empty">No matching employees.</td></tr></tbody>
              <tfoot><tr><th class="btr-grid-person">{{gridSearch.trim()?'FILTERED BTR TOTAL':'BTR TOTAL'}}</th><td v-for="date in cutoffDays" :key="date">{{hours(sheetBtrDateTotal(date))}}</td><th class="btr-grid-total btr-total-covered">{{hours(sheetBtrTotal)}}</th><td class="btr-grid-total">—</td></tr><tr><th class="btr-grid-person">{{gridSearch.trim()?'FILTERED REST TOTAL':'REST TOTAL'}}</th><td v-for="date in cutoffDays" :key="date">{{hours(dateTotal(date))}}</td><td class="btr-grid-total btr-total-covered">—</td><th class="btr-grid-total">{{hours(gridTotal)}}</th></tr></tfoot>
            </table>
          </div>
          <p class="btr-grid-hint">This cutoff has <strong>{{hours(relieverRosterTotal)}} coverage hours</strong>, shown from both the BTR and REST sides — do not add them together. Totals include pending changes; regular and OT hours stay unchanged.</p>
...
.btr-grid .btr-total-covered{right:78px}
.btr-grid .btr-day-col{width:auto}
.btr-grid .btr-grid-day button{position:relative;gap:0;padding:0}
.btr-day-portion{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;width:100%;min-height:53px;padding:7px 2px}
.btr-day-covered{background:#eaf2ff;color:#1f4d8b}
.btr-day-rest{background:#f0f1f3;color:#374151}
.btr-day-portion+.btr-day-portion{border-top:1px solid #c9d3e1}
.btr-grid .btr-grid-day button .btr-rest-label{color:#374151;font-weight:700}
.btr-grid .btr-grid-day button .btr-pending-dot{position:absolute;top:3px;right:3px;color:#8a6200}
.btr-grid .btr-grid-day button.has-warning::after{content:'';position:absolute;bottom:0;left:0;right:0;height:3px;background:#f97316}
.btr-grid .btr-grid-day button:hover .btr-day-portion{filter:brightness(.97)}
</style>
