    ...
    <header class="header">
      <div class="shell header__row">
        ...
        <div class="header__actions">
          ...
          <NuxtLink to="/loginscreen" class="funnel-button funnel-button--outline header__desktop-action">Login</NuxtLink>
          <NuxtLink to="/signupscreen" class="funnel-button header__desktop-action">Signup <span>→</span></NuxtLink>
    ...
    <main>
      <section id="home" class="hero shell">
        <div class="hero__copy reveal is-visible">
          ...
          <div class="hero__cta">
            <NuxtLink to="/signupscreen" class="funnel-button funnel-button--large">Get started <span>→</span></NuxtLink>
      ...
      <section class="cta shell reveal"><div><span class="section-kicker">Ready when you are</span><h2>Make your next payroll<br /><em>your easiest one yet.</em></h2></div><NuxtLink to="/signupscreen" class="funnel-button funnel-button--light funnel-button--large">Create an account <span>→</span></NuxtLink></section>
    </main>
...
<style scoped>
h1,h2,h3,p { margin:0; } h1,h2 { letter-spacing:-.045em; line-height:1.05; } h1 { font-size:clamp(2.9rem,5.2vw,5rem); } h2 { font-size:clamp(2.25rem,4vw,3.55rem); } h1 em,h2 em { color:var(--accent); font-family:'Playfair Display',serif; font-weight:600; } .shell { width:min(1160px,100%); margin:auto; padding-inline:28px; }
.funnel-button { display:inline-flex; align-items:center; justify-content:center; gap:9px; min-height:42px; padding:10px 17px; border:1px solid transparent; border-radius:8px; color:#fff; background:var(--accent); font-size:.9rem; font-weight:700; line-height:1; text-decoration:none; cursor:pointer; box-shadow:0 10px 22px color-mix(in srgb,var(--accent) 25%,transparent); transition:transform .2s ease,box-shadow .2s ease,background .2s ease,border-color .2s ease; } .funnel-button:hover { transform:translateY(-2px); box-shadow:0 14px 28px color-mix(in srgb,var(--accent) 36%,transparent); } .funnel-button--outline { color:var(--ink); border-color:var(--line); background:transparent; box-shadow:none; } .funnel-button--outline:hover { color:var(--accent); border-color:var(--accent); box-shadow:none; } .funnel-button--light { color:var(--navy); background:#fff; box-shadow:none; } .funnel-button--large { min-height:50px; padding:14px 22px; font-size:.96rem; }
...
.nav { display:flex; gap:30px; align-items:center; color:var(--muted); font-size:.91rem; font-weight:600; } .nav a { position:relative; } .nav > a:not(.nav__mobile-link):not(.nav__mobile-cta)::after { content:''; position:absolute; bottom:-7px; left:0; width:0; height:2px; background:var(--accent); transition:width .2s; } .nav > a:hover::after { width:100%; } .header__actions { display:flex; align-items:center; gap:10px; }.theme-toggle,.menu-toggle { border:1px solid var(--line); background:var(--surface); color:var(--ink); display:grid; place-items:center; cursor:pointer; }.theme-toggle { width:38px; height:38px; border-radius:50%; }.theme-toggle svg { width:18px; }.menu-toggle,.nav__mobile-link,.nav__mobile-cta { display:none; }
...
.cta { margin-top:0; padding:68px 70px; display:flex; justify-content:space-between; align-items:center; gap:32px; color:#fff; background:linear-gradient(115deg,#173472,#2869da); border-radius:0 0 7px 7px; }.cta .section-kicker { color:#b8d1ff; }.cta h2 { margin-top:14px; }.cta h2 em { color:#fff; }.footer { padding:34px 0; background:var(--surface); }.footer__row { display:flex; align-items:center; justify-content:space-between; gap:20px; color:var(--muted); font-size:.75rem; }.footer__links { display:flex; gap:21px; font-weight:600; }.footer__links a:hover { color:var(--accent); }
.reveal { opacity:0; transform:translateY(24px); transition:opacity .7s ease var(--delay,0ms),transform .7s cubic-bezier(.2,.7,.2,1) var(--delay,0ms); }.reveal.is-visible { opacity:1; transform:none; } @keyframes float { 50% { transform:translateY(-9px); } } @keyframes spin { to { transform:rotate(360deg); } } @media (prefers-reduced-motion:reduce) { .reveal { opacity:1; transform:none; } }
